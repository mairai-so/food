...
const GRUPOS_MENU: { titulo: string; itens: { href: string; label: string }[] }[] = [
  ...
  },
  {
    ...
    itens: [
      ...
      { href: '/central-de-lojas', label: 'Central de Lojas' },
    ...
    <div translate="no" className="flex h-screen flex-col overflow-hidden bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      ...
      <header className="flex shrink-0 items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 px-4 py-2">
        <div className="flex items-center gap-3">
          <PainelLogo me={me} />
          <div>
              <p className="text-[10px] uppercase tracking-[0.25em] text-emerald-500 dark:text-emerald-400">Miar Supergestora</p>
            <p className="text-sm font-semibold leading-none">{me?.restaurantName ?? 'Restaurante'}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {(me as any)?.isPlatformAdmin && (
            <Link href="/ativacoes" className="rounded-lg bg-amber-400 px-3 py-1.5 text-sm font-semibold text-slate-950 hover:bg-amber-300">
              App Mestre
            </Link>
          )}
...
function Router() {
  ...
  return (
    <Switch>
      <Route path="/" component={AtivacoesSuperAdmin} />
      <Route path="/termos" component={TermosPage} />
      <Route path="/privacidade" component={PrivacidadePage} />
      <Route path="/painel" component={Dashboard} />
      <Route path="/ativacoes" component={AtivacoesSuperAdmin} />
      ...
      <Route path="/catalogo" component={Catalogo} />
            ...
            <Route path="/central-de-lojas" component={CentralDeLojas} />
