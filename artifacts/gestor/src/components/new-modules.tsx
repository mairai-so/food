...
const SEGMENT_ICONS: Record<string, string> = {
  pizzaria: '🍕', churrascaria: '🔥', restaurante: '🍽️', bar: '🍻', japones: '🍣',
  hamburgueria: '🍔', cafeteria: '☕', padaria: '🥖', sorveteria: '🍨', marmitaria: '🍱',
  pastelaria: '🥟', 'food-truck': '🚚', adega: '🍷', 'drive-thru-bebidas': '🥤', outros: '✨',
};

const SEGMENTS = SEGMENTOS.map((segmento) => ({
  id: segmento.id,
  name: segmento.nome,
  icon: SEGMENT_ICONS[segmento.id] ?? '🍴',
  description: segmento.descricao,
}));
...
export function SegmentOnboarding() {
  ...
  const applyPreset = async () => {
    ...
    setResult(null);
    try {
      // Multi-loja (15/08/2026): manda x-loja-id igual Estoque/Funcionários,
      // pra o preset entrar vinculado à loja certa quando cardapioPorLoja
      // estiver ativo (sem efeito quando estiver no modo compartilhado).
