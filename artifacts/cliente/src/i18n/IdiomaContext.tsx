...
const CHAVE_LOCALSTORAGE_LEGADA = 'miar-cliente-idioma';
...
export function IdiomaProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    try { token = window.localStorage.getItem('miar_client_token'); } catch { /* ignora */ }
    if (!token) return;
    let cancelado = false;
  ...
  const LANG: Record<Idioma, string> = { pt: 'pt-BR', es: 'es', gn: 'gn', en: 'en' };
  ...
  const setIdioma = useCallback((i: Idioma) => {
    ...
    document.documentElement.lang = LANG[i] ?? i;
  }, []);
