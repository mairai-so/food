...
// RECRIADO 31/07/2026: essa parte tinha sido perdida numa mesclagem de zip
// que sobrescreveu o arquivo. Contrato confirmado contra o teste existente
// em refresh-token.test.ts.
 ...
 */
export function startTokenRefreshLoop(options: StartTokenRefreshLoopOptions = {}): () => void {
  const intervalMs = options.intervalMs ?? 90 * 60 * 1000;
  const refreshUrl = options.refreshUrl ?? "/auth/refresh";
  const fetchImpl = options.fetchImpl ?? fetch;

  const tick = async () => {
    if (!_authTokenGetter) return;
    const currentToken = await _authTokenGetter();
    ...
    try {
      ...
      if (!res.ok) return; // se falhar, tenta de novo no próximo ciclo
      const data = (await res.json()) as { token?: string };
      if (data.token && _authTokenSetter) {
        _authTokenSetter(data.token);
      }
    } catch {
      // rede indisponível — tenta de novo no próximo ciclo, não derruba o app
    }
  };
...
export async function customFetch<T = unknown>(
  ...
  if (!headers.has("authorization")) {
    ...
    if (_authTokenGetter) {
      ...
      token = window.localStorage.getItem("miar_token");
    }
  ...
  // Apply any extra headers registered via setExtraHeadersGetter.
